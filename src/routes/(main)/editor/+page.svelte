...
<Textarea id="notes" placeholder="Write other details here" />
...
<div class="workoutEditor">
	<div>
		<Button variant="outline" on:click={() => handleThing('Series')}>Series</Button>
		<Button variant="outline" on:click={() => handleThing('Exercise')}>Exercise</Button>
	</div>
	<div
	...
	>
		{#each items as item (item.id)}
			<div animate:flip={{ duration: flipDurationMs }}><SeriesCard /></div>
		{/each}
	</div>
</div>
